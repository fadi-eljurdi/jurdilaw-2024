...
function getFolderFromUrl(url) {
    ...
    // Check if there's a path after the domain (avoiding empty strings)
    if (pathParts.length > 1) {
        // Return the first element (assuming it's the folder)
        return pathParts[1];
    } else {
        // No folder found
        return "";
...
function getYoutubeIdShortenUrl(url) {
    const regExp = /^.*\/(?:(youtu\.be\/)|(.*?))([^?]*)/;
    const match = url.match(regExp);

    return match && match[3];
}
function getYoutubeLiveId(url) {
    const regExp = /^.*\/live\/(.*?)$/;
    const match = url.match(regExp);
...
function getDriveFileId(driveLink) {
    // Regex pattern to match the file ID
    var fileIdPattern = /\/file\/d\/([^\/\?]+)/;

    // Extract the file ID using the regex pattern
    var match = driveLink.match(fileIdPattern);
...
function getGoogleDocId(docsLink) {
    // Regex pattern to match the file ID
    var fileIdPattern = /\/document\/d\/([^\/\?]+)/;

    // Extract the file ID using the regex pattern
    var match = docsLink.match(fileIdPattern);
...
function getWorkspaceId(workspaceLink) {
    // Regex patterns for each type of Google workspace link
    var fileIdPatterns = {
        document: /\/document\/d\/([^\/\?]+)/,
        presentation: /\/presentation\/d\/([^\/\?]+)/,
        spreadsheet: /\/spreadsheets\/d\/([^\/\?]+)/,
        forms_long: /\/forms\/d\/e\/([^\/\?]+)/,
        forms_short: /\/([^\/\?]+)/
    };

    // Function to extract file ID based on the type of workspace
    function extractFileId(link, pattern) {
        var match = link.match(pattern);
        return match && match[1] ? match[1] : null;
    }

    // Iterate through each type of workspace link and extract the file ID
    for (var type in fileIdPatterns) {
        var fileId = extractFileId(workspaceLink, fileIdPatterns[type]);
        if (fileId) {
            return fileId; // Return the extracted file ID
        }
    }
...
function getCanvaId(url) {
    // Regular expression to match Canva design IDs in the URL
    const canvaRegex = /\/design\/([^\/?#&]+)/;
...
function getSymbols(category = false) {
    var data = {
        "arrows": [
            {
                "name": "left arrow",
